...
<?php
    ...
    $select = $con -> prepare("SELECT MAX(id_clase) + 1 AS id FROM tipo_vehiculo"); 
    $select-> execute();
    $row = $select->fetch(PDO::FETCH_ASSOC);

    if((isset($_GET["agre"]))&&($_GET["agre"]=="tipu"))
    {
        $tip_vehiculo= $_GET['tip_vehiculo'];


        // validar si la moto ya existe 

        $num_mot= $con->prepare("SELECT * FROM tipo_vehiculo WHERE tip_vehiculo = '$tip_vehiculo'");
        $num_mot-> execute();
        ...
        if ($tip_vehiculo=="")
        {
            ...
            echo '<script>window.location="crear_tip_veh.php"</script>';
        }
        elseif ($motor_num){
            echo '<script>alert("El tip_vehiculo ya existe //CAMBIELO//");</script>';
            echo '<script>windows.location="crear_tip_veh.php"</script>';
        }else
        {
            $registro = $con->prepare("INSERT INTO `tipo_vehiculo` (`id_clase`, `tip_vehiculo`) VALUES (?, ?)");
    
            $registro->execute([NULL, $tip_vehiculo]);
...
<body class="bg-dark d-flex justify-content-center align-items-center vh-100" style="background-color: #C4C4C4;">
    ...
    <div class="bg-white p-5 rounded-5 text-secondary shadow" style="width: 40rem">
            ...
            <div style="height: 5rem; margin-bottom: 2rem;"><h2 class="text-center text-dark fs-1 fw-bold"> Crear Tipo de Vehículo </h2> </div>
                
                <label for="">ID</label>
                <input type="text" class="form-control bg-light" name="id" placeholder="<?php echo $row['id']?>" readonly><br>

                <label for="">Tipo de Vehículo</label>
                <input type="text" class="form-control bg-light" name="tip_vehiculo" pattern="[A-Za-z]{1,15}" maxlength="15" placeholder="Nombre del Tipo"><br>
