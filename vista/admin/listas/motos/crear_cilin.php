...
<?php
    ...
    $select = $con -> prepare("SELECT MAX(id_cilindraje) + 1 AS id FROM cilindraje"); 
    $select-> execute();
    ...
    if((isset($_GET["agre"]))&&($_GET["agre"]=="tipu"))
    {
        ...
        if ($cilindraje=="")
        {
            ...
            echo '<script>window.location="crear_cilin.php"</script>';
        }
        elseif ($motor_num){
        ...
        {
            $registro = $con->prepare("INSERT INTO `cilindraje` (`id_cilindraje`, `cilindraje`) VALUES (?, ?)");
    
            $registro->execute([NULL, $cilindraje]);
...
<body class="bg-dark d-flex justify-content-center align-items-center vh-100" style="background-color: #C4C4C4;">
    ...
    <div class="bg-white p-5 rounded-5 text-secondary shadow" style="width: 40rem">
            ...
            <div style="height: 5rem; margin-bottom: 2rem;"><h2 class="text-center text-dark fs-1 fw-bold"> CREAR CILINDRAJE </h2> </div>
                ...
                <input type="number" class="form-control bg-light" name="cilindraje" min="0" pattern="\d{1,4}" placeholder="Cantidad de Cilindraje"><br>
